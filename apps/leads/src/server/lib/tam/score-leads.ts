...
function computeHeat(
  ...
  const heat = signalCountToHeat(detected.length);
  const heatLabel = HEAT_LABELS[heat];

  return { heat, heatLabel, heatReasons: reasons, signalCount: detected.length };
}

function signalCountToHeat(count: number): HeatLevel {
  if (count >= 3) return "Burning";
...
export function scoreLead(
  ...
  const { tier, tierLabel, tierReasons, matchCount } = computeTier(lead, icp);
...
export function scoreLeads(
  ...
  return leads
    .map(({ lead, signals }) => scoreLead(lead, icp, signals))
    .sort((a, b) => {
