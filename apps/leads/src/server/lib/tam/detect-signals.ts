...
/**
 * Detect all 5 signals for a lead's company.
...
export async function detectAllSignals(
  ...
  const [hiringOutbound, salesLed, funding, techStack, jobChange] =
    await Promise.all([
  ...
  return [hiringOutbound, salesLed, funding, techStack, jobChange];
}
