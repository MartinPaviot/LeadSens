...
export async function syncVariantAttribution(
  ...
  // 3. Batch lookup: all step-0 DraftedEmails for this campaign
  const allDrafted = await prisma.draftedEmail.findMany({
    where: { campaignId, step: 0 },
    select: {
      subject: true,
  ...
  const draftedByEmail = new Map(
    allDrafted.map((d) => [d.lead.email, d]),
  );

  // 4. Match variants in memory, collect attributions
  const attributions: { email: string; variantIndex: number }[] = [];
  const total = firstEmailByLead.size;

  for (const [leadEmail, sentEmail] of firstEmailByLead) {
    if (!sentEmail.subject) continue;

    const drafted = draftedByEmail.get(leadEmail);
    if (!drafted) continue;
    ...
    attributions.push({ email: leadEmail, variantIndex });
  }

  // 5. Batch update in a single transaction
  if (attributions.length > 0) {
    await prisma.$transaction(
      attributions.map(({ email, variantIndex }) =>
        prisma.emailPerformance.updateMany({
          where: {
            campaignId,
            email,
            variantIndex: null,
  ...
  return { attributed: attributions.length, total };
}
