...
export function buildEmailPrompt(params: {
  ...
  tier?: LeadTier;
  /** Thompson-ranked CTA list for this step (replaces static CTA_LIBRARY) */
  ctaRanking?: string[];
}): string {
...
${params.ctaRanking?.length
  ? `## CTAs FOR STEP ${params.step} (ranked by your reply rate — use #1 unless context demands otherwise)
${params.ctaRanking.map((c, i) => `${i + 1}. "${c}"${i === 0 ? " ★" : ""}`).join("\n")}`
