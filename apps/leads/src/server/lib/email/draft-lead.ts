...
import { draftWithQualityGate, getDynamicQualityThreshold } from "@/server/lib/email/quality-gate";
...
import { getSignalRanking, getStepAnnotation, getCtaRanking } from "@/server/lib/analytics/adaptive";
...
/**
 * Extract the CTA (last question or call-to-action sentence) from an email body.
 * Looks for the last sentence ending with "?" first, then falls back to last short sentence.
 */
export function extractCta(body: string): string | null {
  // Normalize escaped newlines to real newlines
  const normalized = body.replace(/\\n/g, "\n");

  // Split into sentences by line breaks and punctuation boundaries
  const lines = normalized.split(/\n+/).map((l) => l.trim()).filter(Boolean);

  // Find the last line that ends with "?" (the CTA question)
...
export async function draftEmailsForLead(
  ...
  for (let step = 0; step < 6; step++) {
    const [stepAnnotation, ctaRanking] = await Promise.all([
      getStepAnnotation(workspaceId, step),
      getCtaRanking(workspaceId, step),
    ]);
    ...
    const { subject, subjects, body, qualityScore } = await draftWithQualityGate({
      draftFn: () =>
        draftEmail({
          ...
          ctaRanking: ctaRanking ?? undefined,
        }),
