...
/** Cache TTL for style samples — 10 min, invalidated on style feedback */
const STYLE_CACHE_TTL = 600;
...
export async function captureStyleCorrection(
  ...
  // Invalidate cached style samples for this workspace
  await cacheInvalidatePattern(`style:${workspaceId}:*`);
}
...
export async function getStyleSamples(
  ...
  const catKey = Array.isArray(category) ? category.join(",") : (category ?? "all");
  const cacheKey = `style:${workspaceId}:${catKey}:${limit}`;
  ...
  const result = corrections.map(
    (c) => `Original: "${c.originalOutput}"\nCorrected: "${c.userEdit}"`,
  );
  await cacheSet(cacheKey, result, STYLE_CACHE_TTL);
  return result;
}
...
export async function getWinningEmailPatterns(
  ...
  // Filter by denormalized workspaceId (no lead JOIN needed)
