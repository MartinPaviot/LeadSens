  ...
  return {
    ...
    enrich_leads_batch: {
      ...
      async execute(args) {
        ...
        // Batches >3 leads are dispatched to Inngest for background processing.
        // The chat returns instantly with a jobId for progress tracking.
        const BACKGROUND_THRESHOLD = 3;
        if (leads.length > BACKGROUND_THRESHOLD) {
          const jobId = crypto.randomUUID();
          await inngest.send({
            name: "leadsens/leads.enrich",
            data: {
          ...
          return {
            ...
            jobId,
            total: leads.length,
            message: `Enrichment of ${leads.length} leads started in background. Emails will be drafted automatically when done.`,
