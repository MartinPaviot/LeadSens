...
async function safeTransition(leadId: string, to: LeadStatus, tx?: Parameters<typeof import("@/server/lib/lead-status").transitionLeadStatus>[2]) {
  try {
    // Import dynamically to avoid circular deps
    const { transitionLeadStatus } = await import("@/server/lib/lead-status");
    await transitionLeadStatus(leadId, to, tx);
  } catch {
...
export async function POST(req: Request) {
  ...
  // ─── Idempotency: reject duplicate webhook deliveries via Redis SET NX ───
  const email = "email" in event ? event.email : "";
  const idempotencyKey = `${event.event_type}:${event.campaign_id}:${email}:${event.timestamp ?? ""}`;
  ...
  switch (event.event_type) {
    case "reply_received": {
      ...
      // All DB writes in a single transaction — prevents race conditions on concurrent webhooks
      await prisma.$transaction(async (tx) => {
        // Update EmailPerformance
        await tx.emailPerformance.upsert({
        ...
        if (replyVariantIndex != null) {
          await tx.emailPerformance.updateMany({
        ...
        const thread = await tx.replyThread.upsert({
          where: { leadId_campaignId: { leadId: lead.id, campaignId: campaign.id } },
          create: {
        ...
        await safeTransition(lead.id, "REPLIED", tx);
      }, { timeout: 10000 });

      // Side effects AFTER commit (they read data written above)
      ...
      // Invalidate correlator cache after reply data changes
    ...
    case "email_bounced": {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await safeTransition(lead.id, "BOUNCED", tx);
      }, { timeout: 10000 });
