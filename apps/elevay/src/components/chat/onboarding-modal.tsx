...
const PLATFORM_CONFIG = {
  ...
  linkedin: {
    name: "LinkedIn",
    logo: { type: "img" as const, src: "https://cdn.simpleicons.org/linkedin/0A66C2" },
    category: "SOCIAL MEDIA" as const,
  },
  instagram: {
    name: "Instagram",
    logo: { type: "img" as const, src: "https://cdn.simpleicons.org/instagram/E4405F" },
    category: "SOCIAL MEDIA" as const,
  },
  tiktok: {
    name: "TikTok",
    logo: { type: "img" as const, src: "https://cdn.simpleicons.org/tiktok/000000" },
    category: "SOCIAL MEDIA" as const,
  },
  facebook: {
    name: "Facebook",
    logo: { type: "img" as const, src: "https://cdn.simpleicons.org/facebook/1877F2" },
    category: "SOCIAL MEDIA" as const,
  },
  x: {
    name: "X",
    logo: { type: "img" as const, src: "https://cdn.simpleicons.org/x/000000" },
    category: "SOCIAL MEDIA" as const,
  },
} satisfies Record<string, {
  name: string;
  logo: { type: "emoji"; value: string } | { type: "img"; src: string };
  category: "PRODUCTIVITY" | "SOCIAL MEDIA";
}>;

type PlatformKey = keyof typeof PLATFORM_CONFIG;
const PLATFORM_KEYS = Object.keys(PLATFORM_CONFIG) as PlatformKey[];
...
export function OnboardingModal({ open, onComplete }: OnboardingModalProps) {
  ...
  // ─── Load social statuses on step 5 ───────────────────

  useEffect(() => {
    if (step !== 5 || socialFetchedRef.current) return;
  ...
  async function handleBrandUrlBlur() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log("[detect] full response:", JSON.stringify(data, null, 2));
        ...
        if (
          ...
          competitors.every((c) => !c.name && !c.url)
        ) {
  ...
  function validateStep3(): boolean {
    ...
    if (priorityChannels.length === 0) {
      errs.priority_channels = "Select at least 1 channel";
    }
    if (Object.keys(errs).length > 0) {
      setErrors(errs);
      return false;
  ...
  return (
    <Dialog open={open} onOpenChange={() => { /* blocked — no close without completion */ }}>
      <DialogContent className="max-w-[560px] p-8" showCloseButton={false}>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-1">Direct download, no account required</div>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="text-xs text-muted-foreground mt-1">Editable document, saved to your Google Drive</div>
                </button>
              </div>
              {exportFormat === "gdoc" && (
                <div className="mt-3">
          ...
          <div className="space-y-5">
            {loadingSocial ? (
              ...
              CATEGORIES.map(category => {
                const platforms = PLATFORM_KEYS.filter(k => PLATFORM_CONFIG[k].category === category);
                return (
                  <div key={category}>
